...
PATH_WEIGHTS = "/home/vinohithreddy/Self/ViWi/weights/model_1_pred.pth"
PATH_EMBED = "/home/vinohithreddy/Self/ViWi/weights/embed_1.txt"
...
with torch.no_grad():
    ...
    for v_batch, beam in enumerate(val_loader):
        ...
        pred_beams = torch.argmax(out, dim=2)
        print(" ", init_beams[0].numpy(), "   ", pred_beams[0][0].numpy(), "\t    ", targ[0][0].numpy())
        batch_score += torch.sum( torch.prod( pred_beams == targ, dim=1, dtype=torch.float ) )
